...
> choice page]{.underline}](http://www.aboutads.info/choices/)" to
> WebChoices.
>
...
> cookie-less^1^/non-cookie technologies for interest-based advertising
> ("IBA") (collectively "non-cookie technologies"). Companies using
> non-cookie tech should integrate with WebChoices 2.0 as it makes
> setting opt outs more transparent and allows opt outs to be set in
> more cases than in the current version.
>
...
> subdomains - [optout.aboutads.info and dev.aboutads.info for testing
> purposes]{.underline}.
>
> Links to access the WebChoices Tool should point to
> [optout.aboutads.info]{.underline}.
>
> CONFIDENTIAL - DO NOT FORWARD OR SHARE
>
> ^1^ Cookie-less technologies in this WebChoices document do NOT
> include IDFA, AAID, hashed emails and phone numbers. DAA offers
...
> [[www.aboutads.info/choices]{.underline}](http://www.aboutads.info/choices),
...
IV. [CONSUMER OPT-OUT INTEGRATION SPECIFICATION](#_TOC_250001)
...
V.  [ESTABLISHING A FIRST PARTY TRUST
...
A.  Identify a list of companies who have committed to conducting
    responsible advertising practices in alignment with the DAA's
    Principles;

B.  Receive a detailed report of which companies engage in
    Interest-Based Advertising on a consumer's browser; and,

C.  Provide consumers the ability to opt out of collection and use of
    data for Interest-Based Advertising from one, several, or all of the
    companies listed on the WebChoices tool.

D.  ~~The Opt-Out functionality flowchart demonstrates the high-level
...
a.  Staging & Onboarding: <http://dev.aboutads.info>

b.  Production:
    [[http://optout.aboutads.info]{.underline}](http://optout.aboutads.info/)

<!-- -->

B.  **Credentials**. The staging environment is essentially a sandbox
...
A.  **Step 1 - Email Contact**: Each company currently listed on the
...
B.  **Step 2 - Respond to Email**: Your organization's POC will receive
    an email from DAA to the email address you have provided. To update
    your contact email, please inform DAA staff directly.

C.  **Step 3**: Provide DAA with the following:

    1.  A URL to your organization's website;

    2.  A URL to your organization's privacy policy or privacy notice;

    3.  A brief description of your company (this will appear on the
        opt-out page);

    4.  A logo of your company (this will appear on the opt-out page);

    5.  A designation of whether you use cookie technologies for IBA;

    6.  A designation of whether you use non-cookie technologies for
        IBA;

    7.  A designation of whether you are able to read the AdChoices
        Signal in the Protect My Choices extension.;

    8.  The (non-unique) value of your opt-out cookie; and,

    9.  The URL where your Advertising ID rotation functionality exists.
...
D.  **Step 6**: After providing the necessary information, we will add
...
E.  **Step 7**: After DAA review of your submission, you will receive a
...
a)  No technologies shall be utilized in your implementation of an
...
a)  Companies shall use generic values for their company\'s opt-out
    cookie value.

b)  Opt-out cookies that are generated by a company endpoint shall have
...
c)  Cookies not required to opt out a user shall never be set by a
...
e)  Companies may not exploit the trusted relationship established via
...
f)  Some companies may have multiple endpoints, each with a different
    opt-out cookie value. Please contact DAA staff if you have multiple
    opt-out cookies with differing values.

g)  Upon a successful opt out, all cookies related to IBA activities
    shall be expired, except those necessary to maintain the opt-out
    status of the consumer.

h)  Non-cookie based IBA technologies, such as IBA made possible by a
...
a)  Companies are allowed (but not required) to exchange anti-CSRF
    tokens in advance of setting an opt-out cookie as part of this
    specification for a consumer opt-out process.

b)  Companies seeking the additional transactions that allow the
...
c)  If a request to an endpoint fails due to Anti-CSRF Token validation,
...
a)  As is the case with the current choice page, all failures/errors
...
a)  Use some variation of 'opt-out' as the name of your company\'s
    opt-out cookie. This helps consumers identify which cookies are
    opt-out cookies.

b)  Use Request Referrers to limit access to a company's endpoint, but
...
a)  Maintaining fast endpoints is becoming increasingly important with
...
b)  The recommended approach for those companies that are not using
...
### Request
  ...
  **Attribute**   **Description / value**
  --------------- ----------------------------------------------------------
  Referer         http://\<dev \| staging \| qa \| integrate \| www \|
                  optout\>.aboutads.info/\*

  Accept          text/html,application/xhtml+xml
  --------------------------------------------------------------------------

> *[Sample Requests URLs:]{.underline}*
>
...
> &nocache=223442
  ...
  **Field**          **Data    **Description**
                     Type**    
  ---------------- ----------- -------------------------------------------------------
  action_id          integer   Value "3" indicates a status request and token if
                               applicable

  participant_id     string    Participant Identifier for the Opt-out tool to
                               correlate your endpoint with operations. Value is
...
+----------------+----------+---------------------------------------------+
| status         | integer  | 1 = No IBA cookie(s) from Participant on    |
|                |          | the browser.                                |
|                |          |                                             |
|                |          | 2 = IBA cookie(s) from Participant present  |
|                |          | on the browser.                             |
|                |          |                                             |
|                |          | 3 = Opt-out cookie is present on the        |
|                |          | browser.                                    |
+----------------+----------+---------------------------------------------+
| token          | string   | optional Anti-CSRF string, Must be valid    |
|                |          | ASCII characters allowed in a URL.          |
+----------------+----------+---------------------------------------------+

> *[Response URL:]{.underline}*
>
> \<**rd**\>/token/\<**participant_id**\>/\<**status**\>/\<**token**\>
> [Example:]{.underline}
> [[**http://www.aboutads.info**/token/**123/1/magic_string**]{.underline}](http://www.aboutads.info/token/123/1/magic_string)

### Response (New Specification)

> This response is required for participants. Companies utilizing
> non-cookie technologies [are]{.underline} [required]{.underline} to
> provide accurate status as specified below.
...
| participant_id | integer  | **participant_id** value from request        |
+----------------+----------+----------------------------------------------+
| cookie-status  | integer  | 0 = Not Applicable - Cookie Technologies are |
|                |          | not in use                                   |
|                |          |                                              |
|                |          | 1 = No IBA cookie(s) from Participant on the |
|                |          | browser                                      |
|                |          |                                              |
|                |          | 2 = IBA cookie(s) from Participant present   |
|                |          | on the browser.                              |
...
|                |          | browser.                                     |
+----------------+----------+----------------------------------------------+
| other-status   | integer  | 0 = Not Applicable - Non-Cookie Technologies |
|                |          | are not in use                               |
|                |          |                                              |
|                |          | 1 = No Non-Cookie IBA Identifier from        |
|                |          | Participant present for the browser          |
|                |          |                                              |
|                |          | 2 = Non-Cookie IBA Identifier from           |
|                |          | Participant present for the browser.         |
|                |          |                                              |
|                |          | 3 = Non-Cookie Opt-out from Participant      |
|                |          | present for the browser.                     |
+----------------+----------+----------------------------------------------+
| token          | string   | optional Anti-CSRF string, Must be valid     |
|                |          | ASCII characters allowed in a URL.           |
+----------------+----------+----------------------------------------------+
...
> [[http://www.aboutads.info/token/123/1-1/csrftoken]{.underline}](http://www.aboutads.info/token/123/1-1/csrftoken)
>
> Example for company using only cookie technology:
> [[http://www.aboutads.info/token/123/1**-0**/csrftoken]{.underline}](http://www.aboutads.info/token/123/1-0/csrftoken)
...
> &token=magic_value&nocache=223442
  ...
  **Field**          **Data    **Description**
  ...
  token              string    optional Anti-CSRF string if returned during the status
                               check
...
+----------------+---------+------------------------------------------------+
| result_id      | integer | 1 = **Success:** after an opt out attempt, the |
|                |         | opt out cookie is present on the user's        |
|                |         | browser                                        |
|                |         |                                                |
|                |         | 2 = **Failure:** after an opt out attempt, the |
|                |         | cookie is not present on the user's browser    |
|                |         |                                                |
|                |         | 3 = **Failure:** anti-CSRF token mismatch      |
|                |         | \[4..\] = Reserved for future use              |
+----------------+---------+------------------------------------------------+
| message        | string  | optional simple ascii text string fully URL    |
|                |         | compliant.                                     |
+----------------+---------+------------------------------------------------+
...
>
> [Example:]{.underline}
> [[http://www.aboutads.info/finish/123/4/1/simple_string]{.underline}](http://www.aboutads.info/finish/123/4/1/simple_string)
...
| participant_id | string  | **participant_id** value from request              |
+----------------+---------+----------------------------------------------------+
| cookie-result  | integer | 0 = **Not Applicable** - Cookie technologies are   |
|                |         | not in use                                         |
|                |         |                                                    |
|                |         | 1 = **Success:** after an opt out attempt, the opt |
|                |         | out cookie is present on the user's browser        |
|                |         |                                                    |
|                |         | 2 = **Failure:** after an opt out attempt, the     |
|                |         | cookie is not present on the user's browser        |
|                |         |                                                    |
|                |         | 3 = **Failure:** anti-CSRF token mismatch \[4..\]  |
|                |         | = Reserved for future use                          |
+----------------+---------+----------------------------------------------------+
| other-result   | integer | 0 = **Not Applicable** - Non-Cookie technologies   |
|                |         | are not in use                                     |
|                |         |                                                    |
|                |         | 1 = **Opt Out Success**: Non Cookie Identifier for |
|                |         | the browser has been received by the Participants  |
|                |         | server                                             |
|                |         |                                                    |
|                |         | 2 = **Opt Out Failure**: Non Cookie Identifier for |
|                |         | the browser was not received by the Participants   |
|                |         | server                                             |
|                |         |                                                    |
|                |         | 3 = **Failure:** anti-CSRF token mismatch \[4..\]  |
|                |         | = Reserved for future use                          |
...
> [Example:]{.underline}
>
> [[**http://www.aboutads.info**/token/**123/1-1/magic_string**]{.underline}](http://www.aboutads.info/token/123/1/magic_string)
...
> URL Parameters
  ...
  **Field**        **Data    **Description**
                   Type**    
  ------------- ------------ ---------------------------------------------
  action_id        string    value "5" indicates establish temporary trust
                             by setting a session cookie.
