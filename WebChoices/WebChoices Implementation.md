...
4.  **Step 6**: After providing the necessary information, we will add
...
5.  **Step 7**: After DAA review of your submission, you will receive a
...
2)  Opt-out cookies that are generated by a company endpoint shall have
...
3)  Cookies not required to opt out a user shall never be set by a
...
4)  The WebChoices tool may give companies an enhanced trust
...
6)  Some companies may have multiple endpoints, each with a different
    opt-out cookie value. Please contact DAA staff if you have multiple
    opt-out cookies with differing values.

7)  Upon a successful opt out, all cookies related to IBA activities
    shall be expired, except those necessary to maintain the opt-out
    status of the consumer.

8)  Non-cookie based IBA technologies, such as IBA made possible by a
...
2)  Companies seeking the additional transactions that allow the
...
3)  If a request to an endpoint fails due to Anti-CSRF Token validation,
...
2)  Use Request Referrers to limit access to a company’s endpoint, but
...
2)  The recommended approach for those companies that are not using
